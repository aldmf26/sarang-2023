...
class GudangController extends Controller
{
    ...
    function export2(gudangcekModel $model)
    {
        ...
        foreach ($gudangbk as $d) {
            ...
            $sheet1->setCellValue('E' . $kolom, $d->pcs);
            $sheet1->setCellValue('F' . $kolom, $d->gr);
            $sheet1->setCellValue('G' . $kolom, round($d->ttl_rp == 0 ? 0 : $d->ttl_rp  / $d->gr, 0));
    ...
    public function getSuntikan($index)
    {
        $datas = [
            ...
            22 => DB::selectOne("SELECT sum(a.pcs) as pcs,sum(a.gr) as gr,sum(a.ttl_rp) as ttl_rp FROM opname_suntik as a WHERE a.ket = 'cetak_stok_awal' and opname = 'T'"),
