...
class OpnameNewController extends Controller
{
    ...
    private function datapengiriman($spreadsheet, $style_atas, $style, $model)
    {
        ...
        $grading = DB::select("SELECT nm_partai, box_pengiriman, grade, sum(pcs) as pcs, sum(gr) as gr, sum(ttl_rp) as ttl_rp, sum(cost_bk) as cost_bk, sum(cost_kerja) as cost_kerja, sum(cost_cu) as cost_cu, sum(cost_op) as cost_op FROM `grading_partai` WHERE sudah_kirim = 'T'  group by box_pengiriman;");
        $kolom = 2;
