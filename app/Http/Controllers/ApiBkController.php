...
class ApiBkController extends Controller
{
    public function sarang(Request $r)
    {
        ...
        foreach ($cabut as $c) {
            ...
            $ttl_rp_cbt +=  $c->selesai == 'Y' ?  $rupiah - $denda_hcr + $eot_bonus + $bonus_susut : $rupiah;
            $ttl_rp_cbt_hilang += $c->selesai == 'Y' ?  '0' : $rupiah - $denda_hcr + $eot_bonus + $bonus_susut;
        ...
        foreach ($cetak as $c) {
            ...
            $gr_awal_ctk +=  $c->selesai == 'T' ? '0' : $c->gr_awal_ctk;
        ...
        $response = [
            ...
            'data' => [
                ...
                'cetak' => [
                    ...
                    'susut' => empty($gr_akhir_ctk) ? '0' : (1 - ($gr_akhir_ctk / $gr_awal_ctk)) * 100,

                ],
                'sortir' => [
                    'pcs_awal' => $pcs_awal_str,
                    'pcs_akhir' => $pcs_akhir_str,
                    'gr_awal' => $gr_awal_str,
                    'gr_akhir' => $gr_akhir_str,
                    'rp_c' => $ttl_str,
                    'rp_c_dibawa' => $ttl_str_dibawa,
                    'rp_gram' => empty($gr_akhir_str) ? '0' : $ttl_str / $gr_akhir_str,
                    'susut' => empty($gr_akhir_str) ? '0' : (1 - ($gr_akhir_str / $gr_awal_str)) * 100,
                ],
            ],
        ];
