...
left join users as g on g.id = a.id_pengawas
    ...
    {
        DB::selectOne("SELECT a.no_box_sortir, sum(b.pcs_awal - d.pcs) as pcs , sum(b.gr_awal - d.gr) as gr ,
        sum(COALESCE(g.ttl_rp,0) + COALESCE(g.cost_cbt,0) + COALESCE(g.cost_eo,0) + COALESCE(g.cost_ctk,0) + COALESCE(g.cost_str,0) + COALESCE(g.cost_cu,0)) as cost_kerja_dll
        FROM grading as a 
