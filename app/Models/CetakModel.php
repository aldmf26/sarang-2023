...
class CetakModel extends Model
{
    ...
    {
        if ($id_pengawas == 0) {
            $result = DB::select("SELECT c.name, d.name as pgws, a.no_box, (a.pcs_akhir + a.pcs_tdk_cetak) as pcs_awal, (a.gr_akhir + a.gr_tdk_cetak) as gr_awal, (e.gr_awal * e.hrga_satuan) as ttl_rp
            FROM cetak_new as a 
            ...
            $result = DB::select("SELECT c.name, a.no_box, (a.pcs_akhir +  a.pcs_tdk_cetak) as pcs_awal, (a.gr_akhir + a.gr_tdk_cetak) as gr_awal, (e.gr_awal * e.hrga_satuan) as ttl_rp
            FROM cetak_new as a 
            left join formulir_sarang as b on b.no_box = a.no_box and b.kategori = 'cetak'
            left join users as c on c.id = b.id_pemberi
            left join bk as e on e.no_box = a.no_box and e.kategori = 'cabut'
    ...
    {
        if ($id_pengawas == 0) {
            $result = DB::select("SELECT a.no_box, c.name, a.pcs_awal_ctk as pcs_awal, a.gr_awal_ctk as gr_awal, (d.gr_awal * d.hrga_satuan) as ttl_rp
            FROM cetak_new as a 
            left join formulir_sarang as b on b.no_box = a.no_box and b.kategori = 'cetak'
            left join users as c on c.id = b.id_pemberi
