...
HAVING a.nm_partai = '$nm_partai';
    ...
    {
        $result = DB::selectOne("SELECT a.nm_partai, sum(a.pcs_awal) as pcs, sum(a.gr_awal) as gr , sum(COALESCE(a.ttl_rp,0) + COALESCE(a.cost_cbt,0) + COALESCE(a.cost_ctk,0) + COALESCE(a.cost_eo,0) + COALESCE(a.cost_str,0)) as ttl_rp
FROM (
SELECT b.nm_partai, a.no_box, a.pcs_akhir as pcs_awal, a.gr_akhir as gr_awal,(b.hrga_satuan * b.gr_awal) as ttl_rp, d.ttl_rp as cost_cbt, e.ttl_rp as cost_ctk, f.name, a.ttl_rp as cost_str, g.ttl_rp as cost_eo
        FROM sortir as a 
        left join bk as b on b.no_box = a.no_box and b.kategori = 'cabut'
        join formulir_sarang as c on c.no_box = a.no_box and c.kategori = 'sortir'
        left join cabut as d on d.no_box = a.no_box
...
Having a.nm_partai = '$nm_partai';
    ...
    {
        $result = DB::selectOne("SELECT a.nm_partai, sum(a.pcs) as pcs, sum(a.gr) as gr , sum(COALESCE(a.cost_bk,0) + COALESCE(a.cost_cbt,0) + COALESCE(a.cost_ctk,0) + COALESCE(a.cost_eo,0) + COALESCE(a.cost_str,0)) as ttl_rp
            FROM (
                SELECT h.name as pemilik, i.name as penerima, c.nm_partai, a.no_box_sortir, b.pcs_awal, b.gr_awal, (b.pcs_awal - j.pcs_ambil) as pcs, (COALESCE(b.gr_awal,0) - COALESCE(j.gr_ambil,0) - COALESCE(k.gr,0)) as gr,
