<table class="table" id="table1">
    <thead>
        <tr>
            ...
            <th class="text-end">Denda Susut %</th>
    ...
    <tbody>
        @foreach ($datas as $no => $d)
            <tr>
                ...
                <td class="text-end">{{ $d->denda_susut }}</td>
                <td class="text-end">{{ number_format($d->denda,0) }}</td>
...
<form action="{{ route('kelas.create') }}" method="post">
    ...
    <x-theme.modal title="Tambah Paket Cabut" size="modal-lg" idModal="tambah">
        <table class="table table-striped">
            ...
            <tbody>
                <tr>
                    ...
                    <td><input name="denda_susut" type="text" class="form-control"></td>
                    <td><input name="denda" type="text" class="form-control"></td>
                    <td>
        ...
        <table class="table table-striped">
            ...
            <tbody>
                <tr>
                    ...
                    <td><input value="{{ $s->denda_susut }}" name="denda_susut" type="text" class="form-control"></td>
                    <td><input value="{{ $s->denda }}" name="denda" type="text" class="form-control"></td>
                    <td>
                        <select name="lokasi" id="" class="form-control">
                            <option value="">- Pilih Lokasi -</option>
                            @foreach ($lokasi as $d)
                                <option {{$s->lokasi == $d ? 'selected' : ''}} value="{{ $d }}">{{ $d }}</option>
                            @endforeach
                        </select>
                    </td>
                </tr>
