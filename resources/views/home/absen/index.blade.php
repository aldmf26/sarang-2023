<x-theme.app title="{{ $title }}" table="Y" sizeCard="6">
    <x-slot name="cardHeader">
        <ul class="col-12 nav nav-pills float-start">
            <li class="nav-item">
                <a class="nav-link {{ $divisi == 'anak' || !$divisi ? 'active' : '' }}" aria-current="page"
                    href="?divisi=anak">Anak</a>
            </li>
            <li class="nav-item">
                <a class="nav-link {{ $divisi == 'pgws' ? 'active' : '' }}" aria-current="page"
                    href="?divisi=pgws">Pgws</a>
            </li>
        </ul>
        <hr style="border: 2px solid #435EBE">

        <div class="col-lg-12">
            <h6 class="float-start mt-1">{{ $title }} {{ $divisi == 'anak' || !$divisi ? 'Anak' : 'Pgws' }}</h6>
        </div>
    ...
    <x-slot name="cardBody">
        ...
        <div x-data="{ divisi: '{{ $divisi }}' }">
            <div x-show="divisi == 'anak' || !divisi">
                            ...
                            <table style="border:1px solid #97a1c3" class="table table-bordered" id="tablealdi">
                                <thead>
                                    <tr>
                                        ...
                                        <th class="dhead ">
                                            <center>
                                                <input type="checkbox" class="form-check toggle-category"
                                                    id="cekSemuaTutup">
                                            </center>
                                        </th>
                                    </tr>
                ...
                <x-theme.modal idModal="loading" btnSave="T" size="modal-lg" disabled="true"
                    title="Tunggu loading">
            ...
            <div x-show="divisi == 'pgws'">
                <p class="text-center">*Pengawas hanya bisa import absensi via Excel</p>
                <p class="text-center">Download Template : <a href="{{ route('absen.template') }}"
                        class="btn btn-sm btn-primary">Template
                        Absensi PGWS</a></p>
                <form action="{{ route('absen.import') }}" method="post" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                        <button type="submit" class="btn btn-primary">Import Absen</button>
                    </div>
                </form>
            </div>
        </div>
