        ...
        <div class="container-fluid">
            ...
            @foreach ($halaman as $h)
                <div class="section">
                    ...
                    <h6 class="fw-bold">Pengawas : {{ $h->pemberi }} ~
                        {{ $h->name }} </h6>
                    <div class="row">
                        <div class="col-lg-12">
                            ...
                            <table class="table table-bordered" style="font-size: 13px; border:1px solid black">
                                ...
                                <tbody>
                                    @php
                                        $detail = $detailByPenerima->get((string) $h->id_penerima, []);
                                    ...
                                    @foreach ($detail as $d)
                                        <tr>
                                            ...
                                            @php
                                                $ttlPcs_cbt += $d->pcs_pth_cabut;
                                                $ttlPcs_ctk += $d->pcs_pth_cetak;
                                            @endphp
                                            <td class="text-end">{{ number_format($d->pcs_pth_cabut, 0) }}</td>
                                            <td>

                                                <input type="text" class="form-control form-control-sm text-end"
                                                    name="pcs_pth[]" value="{{ $d->pcs_pth_cetak }}">
                                                <input type="hidden" name="no_box[]" value="{{ $d->no_box }}">
                                            </td>
                                            <td class="text-end">
                                                {{ number_format($d->pcs_pth_cetak + $d->pcs_pth_cabut, 0) }}
                                            </td>
                                        </tr>
                                    @endforeach
                                </tbody>
