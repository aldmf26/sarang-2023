                            ...
                            <td>
                                @php
                                    ...
                                    $getBox = DB::table('formulir_sarang')
                                        ->where([['no_invoice', $no_invoice], ['kategori', 'wip2'], ['selesai', 'Y']])
                                        ->pluck('no_box')
                                        ->toArray();
                                ...
                                @if (!$getBox)
                                    ...
                                    <a onclick="return confirm('Yakin diselesaikan ?')"
                                        href="{{ route('gudangsarang.selesai_po_wip2', ['no_invoice' => $d->no_invoice]) }}">
                                        <span class="badge bg-success">Selesai</span>
                                    </a>
                                @else
                                    <a href="{{ route('gudangsarang.print_po_wip2', ['no_invoice' => $d->no_invoice]) }}"
                                        target="_blank">
