<x-theme.app title="{{ $title }}" table="Y" sizeCard="12">
    <x-slot name="cardHeader">
        <div class="row justify-content-end">
            ...
            <div class="col-lg-6">
                <a href="{{ route('cabut.export', ['bulan' => $bulan, 'tahun' => $tahun]) }}"
                    class="disabled float-end btn btn-sm icon icon-left btn-primary me-2">
                ...
                <section class="row">
                    <table style="border:1px solid #97a1c3" class="table table-bordered" id="tblAld2"
                        ...
                        <thead>
                            ...
                            <tr>
                                <th class="dhead text-center">Pcs <br>
                                    ({{ number_format($ttlPcsBk, 0) }})
                                </th>
                                <th class="dhead text-center">Gr <br>
                                    ({{ number_format($ttlGrBk, 0) }})
                                </th>
                                <th class="dhead text-center">Pcs <br>
                                    ({{ number_format($ttlPcsAwal, 0) }})
                                </th>
                                <th class="dhead text-center">Gr <br>
                                    ({{ number_format($ttlGrAwal, 0) }})
                                </th>
                                <th class="dhead text-center">Pcs <br>
                                    ({{ number_format($ttlPcsAkhir, 0) }})
                                </th>
                                <th class="dhead text-center">Gr <br>
                                    ({{ number_format($ttlGrAkhir, 0) }})
                                </th>
                        ...
                        <tbody>
                            @foreach ($eoGroup as $i => $d)
                                <tr x-on:click="openRows.includes({{ $i }}) ? openRows = openRows.filter(item => item !== {{ $i }}) : openRows.push({{ $i }})">
                                    ...
                                    <th class="text-end">{{ number_format($d->pcs_bk ?? 0, 0) }}</th>
                                ...
                                @php
                                    ...
                                    $query = DB::select("SELECT 
                                            a.no_box, 
                                            sum(a.gr_eo_awal) as gr_awal,
                                            SUM(a.gr_eo_akhir) as gr_akhir, 
                                            c.pcs_bk, 
                                            c.gr_bk,
                                            sum(a.ttl_rp) as ttl_rp,
                                            sum((1 - a.gr_eo_akhir / a.gr_eo_awal) * 100) as susut
                                            FROM eo as a
                                            ...
                                            WHERE  a.id_pengawas = '$id' AND a.no_box != 9999 AND a.penutup = 'T' AND a.bulan_dibayar = '$bulan' AND YEAR(a.tgl_input) = '$tahun'
                                            GROUP by a.no_box");
                                @endphp
                                @foreach ($query as $x)
                        <tbody x-show="openRows.includes({{ $i }})">
                            ...
                            <tr>
                                ...
                                <td align="right"><a class="detail" target="_blank"
                                        href="{{ route('dashboard.detail', ['sortir',$x->no_box]) }}">{{ number_format($x->no_box, 0) }}
                                        <i class="me-2 fas fa-eye"></i></a></td>
                                <td align="right">{{ number_format($x->pcs_bk ?? 0, 0) }}</td>
                                <td align="right">{{ number_format($x->gr_bk, 0) }}</td>
                                <td align="right">{{ number_format($x->pcs_awal ?? 0, 0) }}</td>
