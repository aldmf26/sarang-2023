<x-theme.app title="{{ $title }}" table="Y" sizeCard="10">
    <x-slot name="cardHeader">
        ...
        <x-theme.button href="#" modal="Y" idModal="import" icon="fa-upload" addClass="float-end"
            teks="Import" />
        {{-- <x-theme.btn_filter /> --}}
        <form action="{{ route('hariandll.import') }}" method="post" enctype="multipart/form-data">
            @csrf
            <x-theme.modal title="Import Harian Dll" idModal="import" btnSave="Y">
                <div class="row">
                    <div class="col-lg-12">
                        ...
                        <tr>
                            ...
                            <td>
                                <select required name="id_anak[]" class="form-control select2" id="">
                                    ...
                                    @foreach ($anak as $d)
                                        <option value="{{ $d->id_anak }}">{{ $d->name }} - {{ strtoupper($d->nama) }}</option>
                                    @endforeach
                                </select>
                            </td>
                            <td>
